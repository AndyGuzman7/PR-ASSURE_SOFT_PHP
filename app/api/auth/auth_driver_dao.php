...
class AuthDriverDAO {
    ...
    public function logInDriver($username, $password) {
        $queryEmail = "SELECT * FROM `driver` WHERE username = :username;";
		$paramsEmail = ['username' => $username];
        ...
        $queryUserDriver = "SELECT D.iddriver AS 'id', P.fullName AS 'name', D.role AS 'role', P.cellphone AS 'cellphone' 
                            ...
                            WHERE D.username = :username AND D.password = MD5(:password) AND D.status = 1;";
        $paramsUser = [
            'username'=>$username,
            'password'=>$password
        ];
